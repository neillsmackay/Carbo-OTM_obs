...
# 
# Interior T&S: ECCO interpolated
# 
# Surface heat and freshwater fluxes: ECCO interpolated
# 
...
ERA5_data = xr.open_mfdataset(mylist[1], decode_times=True, decode_cf=True).sel(time=slice(t1,t2)).sortby('latitude',ascending=True).chunk(chunks = chunks2)
...
AW_data = xr.open_mfdataset(mylist[10]).isel(time=slice((int(t1[0:4])-1985)*12, (int(t2[0:4])-1985)*12 + int(t2[5:7]))).chunk(chunks = chunks3) # Data start in 1985
ECCO_data = xr.open_mfdataset(mylist[12], decode_times=True, decode_cf=True).sel(time=slice(t1,t2)).chunk(chunks = chunks)
...
# Rearrange ECCO data to -180:180 longitude

lon_ECCO = np.hstack((np.arange(1,181),np.arange(-179,1)))
ECCO_data = ECCO_data.assign_coords(lon = lon_ECCO).sortby('lon',ascending=True)
...
area = xr.open_mfdataset(mylist[7]).areas*land_mask.isel(depth=0).values
...
## Specify T,S
ECCO_T = ECCO_data.THETA*(~land_mask.values-1)/(~land_mask.values-1)
ECCO_S = ECCO_data.SALT*(~land_mask.values-1)/(~land_mask.values-1)


## Specify fluxes
ECCO_hf = ECCO_data.hflux*area.values # W
ECCO_wfo = ECCO_data.fwflux*area.values # kg/s
LS_Cflux = LS_CO2flux_interp*area.values # mmol-C/s
AW_Cflux = AW_CO2flux_interp*area.values # mmol-C/s
#DF_Cflux = DF_CO2flux_interp*area.values # mmol-C/s
#DF_Cflux_ub = DF_CO2flux_ub_interp*area.values # mmol-C/s
#DF_Cflux_lb = DF_CO2flux_lb_interp*area.values # mmol-C/s
...
ECCO_hf_3D = ECCO_hf.expand_dims({'depth':depth},axis=0).assign_coords(depth=DIC_data.depth).chunk("auto") # units: W
ECCO_wfo_3D = ECCO_wfo.expand_dims({'depth':depth},axis=0).assign_coords(depth=DIC_data.depth).chunk("auto") # units: kg/s
...
#DF_Cflux_3D = DF_Cflux.expand_dims({'depth':depth},axis=0).assign_coords(depth=DIC_data.depth).chunk("auto") # units: mmol-C/s
#DF_Cflux_ub_3D = DF_Cflux_ub.expand_dims({'depth':depth},axis=0).assign_coords(depth=DIC_data.depth).chunk("auto") # units: mmol-C/s
#DF_Cflux_lb_3D = DF_Cflux_lb.expand_dims({'depth':depth},axis=0).assign_coords(depth=DIC_data.depth).chunk("auto") # units: mmol-C/s
...
volcello_flattened = (volume.stack(z=("latitude","longitude","depth")))
vol_500_flattened = (vol500.stack(z=("latitude","longitude","depth")))
bigthetao_flattened = (ECCO_T.stack(z=("lat","lon","depth")))
so_flattened = (ECCO_S.stack(z=("lat","lon","depth")))
dic_flattened = (dic.stack(z=("latitude","longitude","depth")))
cstar_flattened = (cstar.stack(z=("latitude","longitude","depth")))
areacello_flattened = (dSArea_3D.stack(z=("latitude","longitude","depth")))
mask_flattened = (EN4_mask.mask_EN4.stack(z=("lat","lon","depth")))
hfds_flattened = ECCO_hf_3D.stack(z=("lat","lon","depth"))
wfo_flattened = ECCO_wfo_3D.stack(z=("lat","lon","depth"))
Cflux1_flattened = LS_Cflux_3D.stack(z=("latitude","longitude","depth"))
Cflux2_flattened = AW_Cflux_3D.stack(z=("latitude","longitude","depth"))
#Cflux3_flattened = DF_Cflux_3D.stack(z=("latitude","longitude","depth"))
#Cflux3_ub_flattened = DF_Cflux_ub_3D.stack(z=("latitude","longitude","depth"))
#Cflux3_lb_flattened = DF_Cflux_lb_3D.stack(z=("latitude","longitude","depth"))
...
HF = hfds_flattened
WFO = wfo_flattened
CF1 = Cflux1_flattened
CF2 = Cflux2_flattened
#CF3 = Cflux3_flattened
#CF3ub = Cflux3_ub_flattened
#CF3lb = Cflux3_lb_flattened
...
#Cflux3_sum_hist = np.zeros((nbasins, window,2**tree_depth))
#Cflux3_ub_sum_hist = np.zeros((nbasins, window,2**tree_depth))
#Cflux3_lb_sum_hist = np.zeros((nbasins, window,2**tree_depth))
...
#da_Cflux3_sum_hist = xr.DataArray(data = Cflux3_sum_hist, dims = ["Basin", "Time","Depth"],
#                           coords=dict(Basin = Basins, Time = time_array, Depth=np.arange(2**tree_depth)),
#                        attrs=dict(description="DF Carbon Flux", units="mmol-C/s", variable_id="Cflux3"))
#da_Cflux3_ub_sum_hist = xr.DataArray(data = Cflux3_ub_sum_hist, dims = ["Basin", "Time","Depth"],
#                           coords=dict(Basin = Basins, Time = time_array, Depth=np.arange(2**tree_depth)),
#                        attrs=dict(description="DF Carbon Flux upper bound", units="mmol-C/s", variable_id="Cflux3_ub"))
#da_Cflux3_lb_sum_hist = xr.DataArray(data = Cflux3_lb_sum_hist, dims = ["Basin", "Time","Depth"],
#                           coords=dict(Basin = Basins, Time = time_array, Depth=np.arange(2**tree_depth)),
#                        attrs=dict(description="DF Carbon Flux lower bound", units="mmol-C/s", variable_id="Cflux3_lb"))
...
#ds_BSP['Cflux3_sum_hist'] = da_Cflux3_sum_hist
#ds_BSP['Cflux3_ub_sum_hist'] = da_Cflux3_ub_sum_hist
#ds_BSP['Cflux3_lb_sum_hist'] = da_Cflux3_lb_sum_hist
...
if member=='ens':
    ds_BSP.to_netcdf(mylist[8] + 'ECCO/' + cstardef + '_Cstar_components/BSP_ECCO_NN_%i_%i.nc' %(ti*window, (ti+1)*window-1))
else:
    ds_BSP.to_netcdf(mylist[8] + 'ECCO/' + cstardef + '_Cstar_components/member_' + str(member) + '/BSP_ECCO_NN_%i_%i.nc' %(ti*window, (ti+1)*window-1))
