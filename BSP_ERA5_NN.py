...
# Interior T&S: EN4
# 
# Surface heat and freshwater fluxes: ERA5
# 
...
# Rearrange ERA5 data to -180:180 longitude

lon_ERA5 = np.hstack((np.arange(0,180.25,0.25),np.arange(-179.75,0,0.25)))
ERA5_data = ERA5_data.assign_coords(longitude = lon_ERA5).sortby('longitude', ascending=True)
...
ERA5_mask = xr.open_dataset('/home/users/nmackay/MTM/ERA5_land_sea_mask.nc').lsm
ERA5_mask = xr.where(ERA5_mask.isel(time=0)>0,0,1)
ERA5_mask = xr.concat((ERA5_mask.sel(longitude=slice(180.25,360)),ERA5_mask.sel(longitude=slice(0,180))),dim='longitude').sortby('latitude')


# Interpolate

ERA5_hflux_interp = xr.zeros_like(DIC_data.temperature.isel(depth=0)).rename(new_name_or_name_dict='heat flux')
ERA5_fwflux_interp = xr.zeros_like(DIC_data.temperature.isel(depth=0)).rename(new_name_or_name_dict='freshwater flux')
...
ERA5_data_subset = ERA5_data.fillna(0).isel(time=slice(ti*window, (ti+1)*window))
regridder = xe.Regridder(ERA5_data_subset.ssr,ds_out,"conservative", periodic=True)
ERA5_hflux_interp[ti*window:(ti+1)*window,:,:] = regridder((ERA5_data_subset.ssr+ERA5_data_subset.str+ERA5_data_subset.slhf+ERA5_data_subset.sshf)*ERA5_mask.values, keep_attrs=True).values # J/m^2/day
ERA5_fwflux_interp[ti*window:(ti+1)*window,:,:] = regridder((ERA5_data_subset.tp+ERA5_data_subset.e+ERA5_data_subset.ro)*ERA5_mask.values, keep_attrs=True).values # m/day
...
ERA5_hflux_interp = ERA5_hflux_interp.chunk(chunks = chunks3) # J/m^2/day
ERA5_fwflux_interp = ERA5_fwflux_interp.chunk(chunks = chunks3) # m/day
LS_CO2flux_interp = LS_CO2flux_interp.chunk(chunks = chunks3) #
# Fill in gaps in Watson air-sea flux product

AW_CO2flux_interp = xr.where(LAT>=65, LS_CO2flux_interp.values, AW_CO2flux_interp)
...
## Specify fluxes
ERA5_hf = ERA5_hflux_interp/(24*3600)*area # W
ERA5_wfo = ERA5_fwflux_interp/(24*3600)*area*rho0 # kg/s
...
ERA5_hf_3D = ERA5_hf.expand_dims({'depth':depth},axis=0).assign_coords(depth=DIC_data.depth).chunk("auto") # units: W
ERA5_wfo_3D = ERA5_wfo.expand_dims({'depth':depth},axis=0).assign_coords(depth=DIC_data.depth).chunk("auto") # units: kg/s
...
hfds_flattened = ERA5_hf_3D.stack(z=("latitude","longitude","depth")).chunk("auto")
wfo_flattened = ERA5_wfo_3D.stack(z=("latitude","longitude","depth")).chunk("auto")
...
if member=='ens':
    ds_BSP.to_netcdf(mylist[8] + 'ERA5/' + cstardef + '_Cstar_components/BSP_ERA5_NN_%i_%i.nc' %(ti*window, (ti+1)*window-1))
else:
    ds_BSP.to_netcdf(mylist[8] + 'ERA5/' + cstardef + '_Cstar_components/member_' + str(member) + '/BSP_ERA5_NN_%i_%i.nc' %(ti*window, (ti+1)*window-1))
